...
population_parameters = {'beta': xp.array(np.ones(N)*beta), 'alpha': xp.array(np.ones(N)*alpha), 'mmin': xp.array(np.ones(N)*mmin) ,'mmax': xp.array(np.ones(N)*mmax), 
                             'mu_g': xp.array(np.ones(N)*mu_g), 'sigma_g': xp.array(np.ones(N)*sigma_g), 
                             'lambda_peak': xp.array(np.ones(N)*lambda_peak),'delta_m': xp.array(np.ones(N)*delta_m), 'name': 'BBH-powerlaw-gaussian'}
