...
model = load_model(path_models+'models/MLP_models/SNR_approximator_combined_{}/model.pth'.format(run), device = device)
print('SNR approxiamtor = SNR_approximator_combined_{}'.format(run))
...
while True:    
    ...
    if mass_distribution == 'POWERLAW_PEAK_SMOOTH':
        threads = threads
        m1_samples = gw_priors_v2.sample_m1_POWERLAW_PEAK_SMOOTH(nxN)
        f = gw_priors_v2.f
        def m2_m1_multiprocess(i):
            m2_new =  np.linspace(4.98, m1_samples[i], 1000)
            cdf_m2_interp = f((m2_new, m1_samples[i]))
            t = rn.random(1)
            return np.interp(t,cdf_m2_interp,m2_new)[0]

        with multiprocessing.Pool(threads) as p:
            m2_samples = list((p.imap(m2_m1_multiprocess,np.arange(nxN))))
        m2_samples = np.array(m2_samples)   

        m1  = m1_samples
        m2  = m2_samples
        distributions = {'mass':'Uniform'}
        _, _, _, _, _, _, _, _, _, theta_jn, _, _, psi, _ , geo_time = gw_priors_v2.draw_prior(int(nxN), distributions)
        
    else: 
        _, m1, m2, _, _, _, _, _, _, theta_jn, _, _, psi, _ , geo_time = gw_priors_v2.draw_prior(int(nxN), distributions)
...
output_df.to_csv(path_data+'run_{}_name_{}_catalog_{}_band_{}_batch_{}_N_{}_SNR_{}_Nelect_{}__Full_para_v1.csv'.format(run, Name, in_out,band, int(batch), int(N), int(SNRth), int(Nselect)))
