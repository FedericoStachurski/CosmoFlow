...
ap.add_argument("-Name", "--Name_file", required=True,
   help="Name of data", default='default_name')
...
ap.add_argument("-type", "--type_data", required=True,
   help="Type of data? options [training, testing] ", default = 'training')
ap.add_argument("-mass_distribution", "--mass_distribution", required=False,
   help="Mass distribution options [uniform, PowerLaw+Peak, PowerLaw, PowerLaw+DoublePeak]", default = 'PowerLaw+Peak')
...
ap.add_argument("-mlp_folder", "--mlp_folder", required=True,
   help="Which MLP approxiamtor do you want (This shoudl be the same as the one used for Approximator and Run)")
...
ap.add_argument("-Om0", "--Om0", nargs='+', required=True,
   help="Baryonic density values: OPTIONS [min, max] or value", default = [0.3])
ap.add_argument("-w0", "--w0", nargs='+', required=True,
   help="EoS parameter values: OPTIONS [min, max] or value", default = [-1.0])
ap.add_argument("-eta", "--eta", nargs='+', required=True,
   help="Luminosity Weighting Parameter values: OPTIONS [min, max] or value", default = [0.1])

# Population setup
ap.add_argument("-gamma", "--gamma", nargs='+', required=True,
   help="gamma values: OPTIONS [min, max] or value", default = [4.86])
ap.add_argument("-k", "--k", nargs='+', required=True,
   help="kappa values: OPTIONS [min, max] or value", default = [2.86])
ap.add_argument("-zp", "--zp", nargs='+', required=True,
   help="zp values: OPTIONS [min, max] or value", default = [2.47])
ap.add_argument("-alpha", "--alpha", nargs='+', required=True,
   help="alpha values: OPTIONS [min, max] or value", default = [3.78])
ap.add_argument("-mmax", "--mmax", nargs='+', required=True,
   help="mmax values: OPTIONS [min, max] or value", default = [250])
ap.add_argument("-mmin", "--mmin", nargs='+', required=True,
   help="mmin values: OPTIONS [min, max] or value", default = [4.98])
ap.add_argument("-mu_g", "--mu_g", nargs='+', required=True,
   help="mu_g values: OPTIONS [min, max] or value", default = [32.27])
ap.add_argument("-sigma_g", "--sigma_g", nargs='+', required=True,
   help="sigma_g values: OPTIONS [min, max] or value", default = [6.76])
ap.add_argument("-lambda_peak", "--lambda_peak", nargs='+', required=True,
   help="lambda_peak values: OPTIONS [min, max] or value", default = [0.1])
ap.add_argument("-beta", "--beta", nargs='+', required=True,
   help="beta values: OPTIONS [min, max] or value", default = [0.81])
ap.add_argument("-delta_m", "--delta_m", nargs='+', required=True,
   help="detla_m values: OPTIONS [min, max] or value", default = [4.8])
ap.add_argument("-sigma_g2", "--sigma_g2", nargs='+', required=False,
   help="sigma_g2 values: OPTIONS [min, max] or value", default = [3])
ap.add_argument("-lambda_peak2", "--lambda_peak2", nargs='+', required=False,
   help="lambda_peak2 values: OPTIONS [min, max] or value", default = [0.66])
ap.add_argument("-mu_g2", "--mu_g2", nargs='+', required=False,
   help="mu_g2 values: OPTIONS [min, max] or value", default = [10])
...
mlp_folder = str(args['mlp_folder'])
...
sigma_g2 = list(map(float,args['sigma_g2']))
lambda_peak2 = list(map(float,args['lambda_peak2']))
mu_g2 = list(map(float,args['mu_g2']))
...
print('MLP model = {}'.format(mlp_folder))
...
print('sigma_g2 = {}'.format(sigma_g2))
print('lambda_peak2 = {}'.format(lambda_peak2))
print('mu_g2 = {}'.format(mu_g2))
print('N = {}'.format(N))
...
sigma_g2 = decide_either_uniform_or_ones(sigma_g2,N) ; lambda_peak2 = decide_either_uniform_or_ones(lambda_peak2,N) ; mu_g2 = decide_either_uniform_or_ones(mu_g2,N)

# Swap values where mu_g < mu_g2
inx_switch = np.where(mu_g < mu_g2)
...
try:
    model, scaler_X, scaler_y = mlp.load_model(model_dir='models/MLP_models/' + mlp_folder, device=device)
except Exception as e:
    raise ValueError('No MLP module found named: {}. Original error: {}'.format(folder, str(e)))
...
# Sampling m1 make mass class
population_parameters = {
    'beta': xp.array(beta), # Uniform distribution: [0.81]*N
    'alpha': xp.array(alpha), # Uniform distribution: [3.78]*N
    'mmin': xp.array(mmin), # Uniform distribution: [4.98]*N
    'mmax': xp.array(mmax), # Uniform distribution: [100]*N
    'mu_g': xp.array(mu_g), # Uniform distribution: [32.27]*N
    'sigma_g': xp.array(sigma_g), # Uniform distribution: [3.88]*N
    'lambda_peak': xp.array(lambda_peak), # Uniform distribution: [0.03]*N
    'delta_m': xp.array(delta_m), # Uniform distribution: [4.8]*N
    'lambda_peak2':xp.array(lambda_peak2),
    'mu_g2':xp.array(mu_g2),
    'sigma_g2':xp.array(sigma_g2),
    'name': 'BBH-powerlaw-gaussian', # Uniform distribution: BBH-powerlaw-gaussian (as a label)
    'mass_distribution': mass_distribution, # mass distribution for m1
}
mass_prior = MassPrior(population_parameters, mgrid = 250)
print('computing p(m1,m2) cdfs')
if population_parameters['mass_distribution'] == 'PowerLaw+Peak':
    pdf_function = mass_prior.powerlaw_plus_peak_smooth_vect
elif population_parameters['mass_distribution'] == 'PowerLaw+DoublePeak':
    pdf_function = mass_prior.powerlaw_plus_peak_smooth_2gaussian_vect
cdfs_m1 = mass_prior.make_cdfs(log_space = False , pdf_function = pdf_function) #Make CDFs from PDFs
...
def select_gal_from_pix(pixels_H0_gamma_para): 
    ...
    if loaded_pixel.empty is False: #if there are galaxies in the pixel
        ...
        if np.sum(weights_gal) == 0.0: ### CHECKs for unstable weights
            print()
...
# rename missed since these are going to be looped over 
missed_H0 = H0
missed_Om0 = Om0
missed_w0 = w0
missed_eta = eta
...
if population_parameters['mass_distribution'] in ['PowerLaw+Peak', 'PowerLaw+DoublePeak']:
    missed_beta = beta
    missed_alpha =  alpha
    missed_mmin = mmin
    missed_mmax = mmax
    ...
    if population_parameters['mass_distribution'] == 'PowerLaw+DoublePeak':
        missed_mu_g2 = mu_g2
        missed_sigma_g2 = sigma_g2
        missed_lambda_peak2 = lambda_peak2
...
while True: 
    ...
    if population_parameters['mass_distribution'] in ['PowerLaw+Peak', 'PowerLaw+DoublePeak']:
        ...
        if population_parameters['mass_distribution'] == 'PowerLaw+DoublePeak':
            repeated_mu_g2 = xp.repeat(missed_mu_g2, select) 
            repeated_sigma_g2 = xp.repeat(missed_sigma_g2, select)
            repeated_lambda_peak2 = xp.repeat(missed_lambda_peak2, select) 
    ...
    if in_out is True:
        ...
        if len(inx_in_gal) > 0: #if the nubmer of galaxies selected is greater than zero, start galaxy selection
            ...
            if len(selected_cat_pixels) >= 1: #if we have selected more or one galaxy
                ...
                a, b = (zmin - z_true_gal) / sigmaz_gal, (zmax - z_true_gal) / sigmaz_gal #sample from truncated 
                #gaussian redshift using the uncertainty 
    ...
    #sample priors GW paramters
    _, _, _, a1, a2, tilt1, tilt2, _, _, theta_jn, phi_jl, phi_12, psi, _, geo_time = gw_priors_v2.draw_prior(int(nxN))
    
    geo_time = utilities._in_sidereal_days(geo_time) # Convert GEocentric time GPS into sidereal day seconds
    
    #Sample  m1:
    samples_m1 = mass_prior.draw_m(Nsamples = select, cdfs = missed_cdfs_m1)
    if name_pop == 'NSBH':
        samples_m2 = np.random.uniform(1,3,nxN)
        
    elif name_pop == 'BBH':
        cdfs_m2 = mass_prior.make_cdfs_m2(xp.array(samples_m1), beta = repeated_beta, mmin = repeated_mmin, delta_m = repeated_delta_m)
        m_vect_m2 = xp.linspace(0, samples_m1[:, None], mass_prior.mgrid, axis=1)
        m_vect_m2 = xp.reshape(m_vect_m2, (len(samples_m1), mass_prior.mgrid))
        samples_m2 = mass_prior.draw_m(1, cdfs_m2, m2_arr = xp.hstack(m_vect_m2))

    ## Temporary 
    bad_inx = np.where(samples_m2>samples_m1)[0]
    
    
    m1z = samples_m1*(1+z) #turn source masses into detector frame masses 
    m2z = samples_m2*(1+z)

    #define dictionary with relevant GW parameters 
    data_dict = {'luminosity_distance':dl, 'mass_1':m1z, 'mass_2':m2z,'a_1': a1, 'a_2': a2,
             'tilt_1': tilt1, 'tilt_2': tilt2,'ra':RA, 'dec':dec,
             'theta_jn':theta_jn, 'phi_jl':phi_jl, 'phi_12':phi_12, 'psi':psi , 'geocent_time':geo_time}  

    GW_data = pd.DataFrame(data_dict) #make GW data frame 
    GWdata_mlp = GW_data[[ 'mass_1', 'mass_2', 'a_1', 'a_2', 'tilt_1', 'tilt_2',
                             'ra', 'dec', 'theta_jn', 'phi_jl', 'phi_12','psi','geocent_time']]
    ### SNR calulcator using MLP ### 
    ypred = mlp.predict(model, GWdata_mlp, scaler_X, scaler_y, device = device) #get the predicted y values (SNR * distance)
    snr_pred = ypred/GW_data[['luminosity_distance']].values#divide by distance to get SNR 
    ...
    GW_data['mu_g2'] = repeated_mu_g2
    GW_data['sigma_g2'] = repeated_sigma_g2
    GW_data['lambda_peak2'] = repeated_lambda_peak2
    ...
    if population_parameters['mass_distribution'] in ['PowerLaw+Peak', 'PowerLaw+DoublePeak']:
        ...
        if population_parameters['mass_distribution'] == 'PowerLaw+DoublePeak':
            new_missed_mu_g2 = missed_mu_g2[inx_new_missed]
            new_missed_sigma_g2 = missed_sigma_g2[inx_new_missed]
            new_missed_lambda_peak2 = missed_lambda_peak2[inx_new_missed]
    ...
    if population_parameters['mass_distribution'] in ['PowerLaw+Peak', 'PowerLaw+DoublePeak']:
        ...
        if population_parameters['mass_distribution'] == 'PowerLaw+DoublePeak':
            missed_mu_g2 = new_missed_mu_g2[inx_missed_H0]
            missed_sigma_g2 = new_missed_sigma_g2[inx_missed_H0]
            missed_lambda_peak2 = new_missed_lambda_peak2[inx_missed_H0]
...
output_df = GW_data[['snr', 'H0', 'Om0', 'w0', 'eta', 'gamma','k','zp',
                     'beta', 'alpha', 'mmax', 'mmin', 'mu_g', 'sigma_g',
                     'lambda_peak','mu_g2', 'sigma_g2', 'lambda_peak2',
                     'delta_m', 'z','luminosity_distance', 'mass_1',
                     'mass_2', 'ra', 'dec','a_1', 'a_2', 'tilt_1', 'tilt_2',
