...
model_l1 = load_model(path_models+'models/new_model/h1_SNR_v1O1/model.pth', device = device)
...
#     snr_opt_network = np.sqrt(snr_pred_v1**2 + snr_pred_l1**2 + snr_pred_h1**2)
    ...
    snrs_obs = np.sqrt((ncx2.rvs(4, snr_opt_network**2, size=nxN, loc = 0, scale = 1)))        
...
#     print(len(repeated_H0))
    ...
    if type_of_data =='training':
        ...
        sys.stdout.write('\r H0 we missed: {} | Nselect = {}'.format(len(missed_H0), nxN))
