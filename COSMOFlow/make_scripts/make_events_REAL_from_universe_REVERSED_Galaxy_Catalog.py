...
print('Filtering out any NaNs in the catalog')
      
catalog = catalog.dropna()
Ncat = len(catalog)      

print('Galaxies in the catalog, Ncat = {}'.format(int(Ncat)))

#Redshift weight     
def weights(z):
    return 1/(1+z)

catalog['weights'] = catalog['z'].apply(weights)

#All sky magntiude threshold

mth = np.max(catalog.m_B) 
print('All sky magnitude threshold, mth = {}'.format(mth))
...
if type_of_data == 'training':
    path_data = r"/data_gwcosmo/galaxy_catalog/training_data/"
    

if type_of_data == 'testing': 
    path_data = r"/data_gwcosmo/galaxy_catalog/testing_data/"
...
def GW_events(Nevent):
    ...
    #Cosmology_lists
    z_list, appm_list, H0_list, Omega_m_list= [], [], [], []
    ...
    while i < Nevent:
        ...
        while True:
            ...
            #sample GW priors
            m1, m2, _, _, _, _, ra, dec, _ = gw_priors.draw_prior()
            
            #compute schecter function for given H0 sample
            pM = priors.p_M(M_grid, H0_sample)

            #sample galaxies from unvierse
            Msample = draw_cumulative_M(1, H0_sample, priors.p_M)[0]
            z = draw_cumulative_z(1, pz_int)[0]

            
            #compute distance and apparent magnitudes
            dl = cosmology.fast_z_to_dl_v2(z,H0_sample)
            appmsample = cosmology.app_mag(Msample, dl)[0]

            
            if  appmsample <= mth: 
                indicies = np.random.choice(Ncat, p = catalog['weights']/np.sum(catalog['weights']) , size = 1)
                sampled_row = catalog.iloc[indicies]
      
                #Sampled redshift from gaussian
                while True:
                    z = np.random.normal(sampled_row.z,sampled_row.sigmaz,size=1)[0]
                    if z > 0:
                        break 
                
                dl = cosmology.fast_z_to_dl_v2(z,H0_sample)
                appmsample = np.array(sampled_row.m_B)[0]
                ra = np.array(sampled_row.ra)[0]
                dec = np.array(sampled_row.dec)[0]
            
            #compute SNR from injection
            rho_obs = gw_SNR.SNR_from_inj( dl, m1*(1+z), m2*(1+z), 0.5, 0.5, 0.5, 0.5, ra, dec, 0, 0, 0, 0, 0)
...
#                 theta_jn_list.append(theta_jn)
    ...
    return H0_list,  dl_list, m1_list, m2_list, RA_list, dec_list, appm_list
