...
def select_gal_from_pix(pixels_H0s):
    ...
    if loaded_pixel.empty is False:
        ...
        nan_dictionary = {'z': [np.nan],'RA': [np.nan],'dec':[np.nan], 'sigmaz': [np.nan], 'mK': [np.nan]}
        return pd.DataFrame(nan_dictionary)
...
#     M_abs =[]
    ...
    if in_out is True: 
        ...
        pix_list = np.random.randint(0,hp.nside2npix(NSIDE), len(missed_H0))
    else: 
...
#         pix_list = np.array(pix_list[inx_in_gal])
        ...
        selected_cat_pixels = pd.concat(selected_cat_pixels, ignore_index=True)
        print(selected_cat_pixels)
        print(np.where(np.isnan(np.array(selected_cat_pixels['z'])))[0])
        gal_selected = selected_cat_pixels.dropna()
        ...
        if any(x is None for x in selected_cat_pixels) is False: 
            ...
            RA = RA_gal
            dec = dec_gal
            app_samples = m_obs_gal
            inx_gal = np.ones(len(z))
