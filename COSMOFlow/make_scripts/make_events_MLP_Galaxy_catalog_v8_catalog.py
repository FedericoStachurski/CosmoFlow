...
model_l1 = load_model(path_models+'models/new_model/v1_SNR_l1O3/model.pth', device = device)
model_h1 = load_model(path_models+'models/new_model/v1_SNR_h1O3/model.pth', device = device)
model_v1.set_device(device)
model_l1.set_device(device)
model_h1.set_device(device)
...
#     MLP_GW_data = pd.DataFrame(MLP_data_dict)
    ...
    ypred_l1 = model_l1.run_on_dataset(x_data_MLP)
    ypred_h1 = model_h1.run_on_dataset(x_data_MLP)

    snr_pred_v1 = ypred_v1.cpu().numpy()/np.array(GW_data['luminosity_distance'])
    snr_pred_h1 = ypred_h1.cpu().numpy()/np.array(GW_data['luminosity_distance'])
    ...
    snrs_obs = np.sqrt((ncx2.rvs(6, snr_opt_network**2, size=nxN, loc = 0, scale = 1)))        
