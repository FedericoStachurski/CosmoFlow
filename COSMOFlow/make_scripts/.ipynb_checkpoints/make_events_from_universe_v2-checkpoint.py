...
def GW_events(Nevent):
    ...
    while i < Nevent:
        ...
        while True: 
            ...
            inx_galaxies = np.random.randint(0,Ngalaxies-1,59)
            ...
            if N_out != len(inx_galaxies):    
                    ...
                    #list_parameters = np.array(parameters_out + parameters_in)
#                     else: 
#                         list_parameters = np.array(list(parameters_out))
            #print(rho_temp)
            if len(rho_temp) == 2 : 
            ...
            if np.size(rho_temp) != 0:
                ...
                if i%batches == 0:
                    print('Events {}/{}, SNR = {}, Mz = {}, z = {}, m_app = {} , H0 = {}, Omega_m = {}'.format(i+1,
                                                                                                          Nevent,
                                                                                                          round(rho_obs,3),
                                                                                                          round(Mz_sample,3), 
                                                                                                          round(z_sample,3), 
...
for j in range(batches):
    if type_of_data == 'training':
        ...
        output_df.to_csv(path_data+'/data_mth_{}_SNRth_{}_{}_batch_{}.dat'.format(int(mth), int(rho_th), int(N/batches),j))
        j += 1
    if type_of_data == 'testing':
        #store events
        print('BATCH = {}'.format(j+1))
        ...
        output_df = pd.DataFrame({'H0': event[0], 'Omega_m' : event[1], 
                                  ...
                                  'z': event[4], 'm_app': event[5],
                                  'RA': event[6], 'dec' : event[7]  })
        output_df.to_csv(path_data+'/data_mth_{}_SNRth_{}_1000_H0_{}_Omega_{}.dat'.format(int(mth), int(rho_th), int(N/batches), round(H0,3),round(Omega_m,3)))
        j += 1    
