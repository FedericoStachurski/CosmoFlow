...
if type_of_data == 'training':
    path_data = r"/data/wiay/federico/PhD/gwcosmoFlow_v3/data_gwcosmo/events_mock_REAL_empty_catalogue_v2/training_data/"
    

if type_of_data == 'testing': 
    path_data = r"/data/wiay/federico/PhD/gwcosmoFlow_v3/data_gwcosmo/events_mock_REAL_empty_catalogue/testing_data/"
...
def GW_events(Nevent):
    ...
    while i < Nevent:
        ...
        while True: 
            ...
            #Msamples = draw_cumulative_M(1, H0_sample, priors.p_M)
            ...
            rho_obs = gw_SNR.SNR_from_inj( dl, m1*(1+z), m2*(1+z), a1, a2, tilt1, tilt2, RA, dec, theta_jn, 0, 0, 0, 0)
...
for j in range(batches):
    if type_of_data == 'training':
        ...
        output_df = pd.DataFrame({'H0': event[0], 'dl': event[1],
                                  'm1': event[2], 'm2': event[3], 'a1': event[4],
                                  'a2': event[5], 'tilt1': event[6], 'tilt2': event[7], 
                                  'RA': event[8], 'dec': event[9], 'theta_jn': event[10], 'z': event[11] })
        output_df.to_csv(path_data+'data_{}_batch_{}.csv'.format(int(N/batches),j+Nbatch))
        j += 1
    if type_of_data == 'testing':
        #store events
        print('BATCH = {}'.format(j+1))
