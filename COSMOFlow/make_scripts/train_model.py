...
def read_data(batch):
    path_name ="data_for_MLP/data_dl_mm/training_data/"
    data_name = "_data_{}_batch_{}.csv".format(50000, batch)
...
GW_data = GW_data[[ 'dl', 'm1z', 'm2z', 'snr']]
...
#     ytrain = ytrain[np.where(~np.isnan(ytrain)[:,1])[0],:]
    ...
    model = create_mlp(input_features=in_features,output_features=out_features,neurons=neurons,layers=layers,activation=activation,
                       out_activation=out_activation, device=device, model_name='SNR_approxiamator_2',use_bn=False)
