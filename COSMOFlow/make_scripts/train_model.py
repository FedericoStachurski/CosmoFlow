...
def read_data(batch):
    ...
    data_name = "_data_full_para_uniform_snr_2.csv".format(batch)
...
#                                                                           'phi_12','polarization','geo_time', 'snr'])
    ...
    ha = HA(GW_data.geo_time, GW_data.RA)

    logsnrs = np.log(GW_data.snr * GW_data.dl)
    GW_data['snr'] = logsnrs
    GW_data['HA'] = ha
    return GW_data
...
#     ytrain = ytrain[np.where(~np.isnan(ytrain)[:,1])[0],:]
    ...
    activation = nn.SiLU
    out_activation=None
    model = create_mlp(input_features=in_features,output_features=out_features,neurons=neurons,layers=layers,activation=activation,out_activation=out_activation, device=device, model_name='SNR_approxiamator_full_para_HA_v9',use_bn=False)
