...
def read_data(batch):
    path_name =r"data_for_MLP/data_sky/training/"
...
#     ytrain = ytrain[np.where(~np.isnan(ytrain)[:,1])[0],:]
    ...
    sched = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max = iterations, eta_min=0, last_epoch=- 1, verbose=False)
    ...
    model_train_test(data, model, device,n_epochs=iterations, n_batches=10, loss_function=loss_function,optimizer=optimizer, verbose=True,update_every=250, n_test_batches=1,save_best=True, scheduler=sched)
