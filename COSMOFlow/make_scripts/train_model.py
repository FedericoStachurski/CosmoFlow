...
#     ytrain = ytrain[np.where(~np.isnan(ytrain)[:,1])[0],:]
    ...
    model = create_mlp(input_features=in_features,output_features=out_features,neurons=neurons,layers=layers,activation=activation,
                       out_activation=out_activation, device=device, model_name='SNR_approxiamator_full_para_HA_v6',use_bn=False)
    ...
    model_train_test(data, model, device,n_epochs=iterations, n_batches=5, loss_function=loss_function,optimizer=optimizer, verbose=True,update_every=100, n_test_batches=1,save_best=True, scheduler=sched)
