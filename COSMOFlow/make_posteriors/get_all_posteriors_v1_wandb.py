...
def load_hyperparameters_scalers_flow(sweep_name, run_name, flow_name):
    ...
    path  = "/data/wiay/federico/PhD/cosmoflow/COSMOFlow/train_flow/"
    #Open hyperparameter dictionary
    def read_config_file(sweep_name, run_name):
        with open(path+"wandb/sweep-"+sweep_name+"/config-"+run_name+".yaml", 'r') as stream:
    ...
    flow_path = path+"wandb/run-"+flow_name+"/files/flow_v1_sweep.pt"
    ...
    path_scalers = path+'wandb/sweep-{}'.format(sweep_name)
    scalerfileX = path_scalers+'/scaler_x.sav'
    scalerfileY = path_scalers+'/scaler_y.sav'
    scaler_x = pickle.load(open(scalerfileX, 'rb'))
    scaler_y = pickle.load(open(scalerfileY, 'rb'))
    
    
#     #
#     scalerfile_x = path+'scalers/scaler_x.sav'
#     scalerfile_y = path+'scalers/scaler_y.sav'
...
for GW_event in events:
    ...
    #Get flow 
    flow, hyper_dict, scaler_x, scaler_y = load_hyperparameters_scalers_flow(sweep_name, run_name,flow_name)
