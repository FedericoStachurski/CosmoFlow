...
def read_data(batch_of_data):
    ...
    data_name ="run_O3_det_['L1', 'V1']_name_control_test_H0-2_catalog_True_band_K_batch_1_N_1000000_SNR_11_Nelect_3__Full_para_v1.csv".format(batch_of_data)
...
list_data = [] 
for i in range(1): #how many batcehs of data to be used, i nthis case 1, (should be user input)
...
scaler_x, scaler_y, scaled_data = utilities.scale_data(data, Scaler, n_conditionals = 1)
...
conditional_val = scaler_y.inverse_transform(np.array(Y_scale_val).reshape(-1,1)) #unscale validation data set 
...
for j in range(n_epochs):
    ...
    for batch in train_loader:
        ...
        loss = -flow.log_prob(target_train.to(device)[:,:n_inputs], conditional=conditionals_train.reshape(-1,1).to(device)).cpu().mean() #compute loss
    ...
    with torch.no_grad(): #freeze all gradients 
        ...
        for batch in val_loader: 
            ...
            loss = - flow.log_prob(target_val.to(device)[:,:n_inputs],conditional=condtionals_val.reshape(-1,1).to(device)).cpu().mean() #perform loss function for validatio ndata set 
    ...
    with torch.no_grad():
        ...
        latent_samples, _= flow.forward(target_data[:,:n_inputs], conditional=conditionals.reshape(-1,1)) #get samples from latent sapce of flow
